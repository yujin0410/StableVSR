...
# Dual-SFT design: frequency-separated conditioning with per-level processors,
# asymmetric encoder/decoder injection (down_blocks[1] LOW + up_blocks[1] HIGH),
# and frequency-separated RGB-space loss (every freq_loss_interval steps).
#
# Train from the StableVSR baseline (no resume from prior checkpoints since
# the architecture is fundamentally different).

MODEL_ID='claudiom4sir/StableVSR'
OUTPUT_DIR='experiments/20260430_dualsft'
GPUS="1 2"
